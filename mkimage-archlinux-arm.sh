#!/usr/bin/env bash
# Generate a minimal filesystem for archlinux and load it into the local docker as "archlinux"
# requires root
set -e
...
case "$arch" in
	...
	*)
		...
		DOCKER_IMAGE_NAME=tokinring/archlinux-arm
		;;
esac
...
expect <<EOF
	...
	expect {
		...
		-exact "installation? \[Y/n\]" { send -- "y\r"; 
exp_continue }
		-exact "delete it? \[Y/n\]" { send -- "y\r"; 
exp_continue }
	}
EOF
